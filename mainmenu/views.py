...
from .models import Library, BookRental, ReviewSite, Motor, Peminjaman, Product, Cart, Order
...
def cart_view(request):
    ...
    # # print(vars())
    # # print(cart.values())
    # # total_price = sum(Decimal(item['price']) * int(item['quantity']) for item in cart.values())
    # # total_price = (print(item['price']) for item in cart.values())
    # for item in cart:
    #     print('X')
    #     print(vars())
    #     # item['price'] = Decimal(item['price'])
    #     # item['total_price'] = Decimal(item['product_price']) * item['quantity']
    #
    # # print(vars())
    # total_price = 2 * 3
    cart_items = CartItem.objects.select_related('product').get()
    print(vars(cart_items))
